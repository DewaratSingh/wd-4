...
import { Shield, User, Mail, Lock, ArrowRight, Loader2, Eye, EyeOff, CheckCircle2 } from "lucide-react";
...
export default function UserSignupPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError("");
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/user/signup`, {
                method: "POST",
                ...
                <div className="relative z-10 px-10 flex-1 flex flex-col justify-center">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.8 }}>
                        <h1 className="text-5xl font-black leading-[1.05] mb-4">
                            ...
                            <span className="text-vibrant-orange">Your City.</span>
                        </h1>
                        <p className="text-indigo-200 text-sm leading-relaxed mb-8 max-w-xs">
                            Join the civic revolution. Report infrastructure issues directly to municipal officers and track resolution in real-time.
                        </p>
            ...
            >
                ...
                <div className="relative z-10 w-full max-w-[400px]">
                    ...
                    <motion.div variants={fade(0)} initial="hidden" animate="visible" className="mb-8">
                        ...
                        <p className="text-slate-500 mt-1.5 text-sm">Join thousands of citizens fixing their cities</p>
                    </motion.div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        {/* Terms */}
                        <motion.div variants={fade(4)} initial="hidden" animate="visible" className="flex items-start gap-3">
                            <input id="agree" type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)}
                                className="mt-0.5 w-4 h-4 rounded border-slate-300 accent-electric-indigo cursor-pointer" />
                            <label htmlFor="agree" className="text-xs text-slate-500 leading-relaxed cursor-pointer select-none">
                                I agree to the{" "}
                                <Link href="/terms" className="text-electric-indigo font-semibold hover:underline">Terms of Service</Link>{" "}
                                and{" "}
                                <Link href="/privacy" className="text-electric-indigo font-semibold hover:underline">Privacy Policy</Link>.
                            </label>
                        </motion.div>
                        ...
                        <motion.div variants={fade(5)} initial="hidden" animate="visible">
                            <button type="submit" disabled={loading}
                                ...
                                {loading ? (
                                    ...
                                    <><span>Create My Account</span><ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>
                                )}
                            </button>
                        </motion.div>
                    </form>
                    ...
                    {/* Footer links */}
                    <motion.div variants={fade(6)} initial="hidden" animate="visible" className="text-center mt-6 space-y-2">
                        <p className="text-sm text-slate-500">
                            Already have an account?{" "}
                            <Link href="/user/login" className="text-electric-indigo font-bold hover:underline">Sign in</Link>
                        </p>
